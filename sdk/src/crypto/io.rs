//! I/O operations for file encryption and decryption.
//!
//! Encryption pipeline: `File -> HashingWriter -> DeflateEncoder -> EncryptingWriter -> SpooledTempFile`.
//!
//! Decryption pipeline: `DecryptingWriter -> InflateWriter -> HashingWriter -> File`.
//!
//! See also [parent module docs](super).

use {
    crate::{content::TemporaryEncryptedFile, crypto::Cipher},
...
struct HashingWriter<W> {
    /// Current hasher state.
    hasher: Sha256,
    /// Current number of processed bytes.
    size: u64,
    /// Inner writer.
    inner: W,
}
...
impl<W> HashingWriter<W> {
    ...
    /// Flush all data and return inner writer, content hash and size in bytes.
...
struct EncryptingWriter<'a, W> {
    ...
    output: W,
    cipher: &'a Cipher,
    /// Current number of generated bytes.
    encrypted_size: u64,
}
...
impl<'a, W: Write> DecryptingWriter<'a, W> {
    ...
    /// Flush all data and return inner writer, content hash, and file size in bytes.
    #[inline]
    pub fn finish(mut self) -> io::Result<(W, ContentHash, u64)> {
        self.process_block()?;
        if !self.buf.is_empty() {
